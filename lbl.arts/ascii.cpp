/*--------------------------------------------------------------------
 * $Id: ascii.c,v 1.1.1.1 2009/12/06 22:38:21 bernhard Exp $
 * 
...
/***********************************************************************************/
/* Function: ASCII_string2double_xy2D                                     @30_30i@ */
/* Description: Convert a two-dimensional array of strings to an x vector          */
/*              (1st column) plus a 2D array y[row][column] (2nd - last column)    */
/* Parameters:                                                                     */
/* Return value:                                                                   */
/*  0  if o.k., <0 if error                                                        */
/*                                                                                 */
...
int ASCII_string2double_xy2D (double *x, 
			      double **y, 
			      char *** string,
			      int rows,
			      int columns)
{
  ...
  /* need at least one column (= x value) */
  if (columns<1)
    return -1;
  ...
  for (i=0; i<rows; i++) {
	...
	y[i][j-1] = strtod (string[i][j], &dummy);
    }
  }
...
/***********************************************************************************/
/* Function: ASCII_file2xy2D                                              @30_30i@ */
...
/*  double *x:          Pointer to a 1D array of double, x [0 ... nx-1]            */
/*  double ***value:    Pointer to a two-dimensional array of double,              */
/*                      y [0 ... nx-1][0 ... ny-1].                                */
/*                                                                                 */
/* Return value:                                                                   */
...
int ASCII_file2xy2D (char *filename,   
		     int *nx, 
		     int *ny, 
		     double **x,
		     double ***y)  
{
  ...
  if (min_columns != max_columns) {
    return NOT_A_RECTANGULAR_MATRIX;
  }    
  ...
  /* no need to do anything if empty file */
  if (max_columns<1)
    return 0;
  ...
  *x = (double *)calloc(*nx, sizeof(double));
  ...
  if ( (status = ASCII_calloc_double (y, *nx, *ny)) != 0 )
    return status;

  /* convert string array to double array */
  if ( (status = ASCII_string2double_xy2D (*x, *y, string, rows, max_columns)) != 0)
    return status; 
...
int ASCII_parse (char *string, char const *separator, char ***array, int *number)
{
...
{
  int i=0, j=0;
  sortarray_struct *temp = (sortarray_struct*)calloc(rows, sizeof(sortarray_struct));
  ...
  for (i=0; i<rows; i++) {
    temp[i].x     = (double*)calloc (columns, sizeof(double));
    temp[i].index = index;
...
{
  int i=0, j=0;
  sortarray_float_struct *temp = (sortarray_float_struct*)calloc(rows, sizeof(sortarray_struct));
  ...
  for (i=0; i<rows; i++) {
    temp[i].x     = (float*)calloc (columns, sizeof(float));
    temp[i].index = index;
